...
def ensure_chat_history_collection(uri: str = MILVUS_URI) -> None:
    ...
    schema.add_field(
        ...
        datatype=DataType.VARCHAR,
        max_length=_SESSION_LEN,
    )
    schema.add_field(
        field_name="checkpoint_id",
        datatype=DataType.VARCHAR,
        max_length=_CHECKPOINT_LEN,
    )
    schema.add_field(
    ...
    schema.add_field(
        field_name="checkpoint_data",
        datatype=DataType.VARCHAR,
        max_length=_DATA_LEN,
    )
    schema.add_field(
        field_name="metadata",
        datatype=DataType.VARCHAR,
        max_length=_META_LEN,
    )
    schema.add_field(
...
class MilvusCheckpointer(BaseCheckpointSaver):
    ...
    def get_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        if checkpoint_id:
            record = self.store.get_by_checkpoint_id(session_id, checkpoint_id)
        else:
            record = self.store.get_latest(session_id)
    ...
    def put(
        ...
        """Persist a checkpoint after each graph node execution."""
        session_id    = config["configurable"]["thread_id"]
    ...
    def clear_session(self, session_id: str) -> int:
        """
        Delete all history for a session — called when user clicks 'New Chat'.
